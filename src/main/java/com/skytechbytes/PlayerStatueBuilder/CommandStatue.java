			...
			try {
				...
				if (flags.contains("slim") || flags.contains("legacy") || flags.contains("default")) {
					if (flags.contains("slim")) {
						new PlayerStatueMaker(p,"slim",bi,false,params).runTask(PlayerStatueBuilder.instance);
					} else if (flags.contains("legacy")) {
						new PlayerStatueMaker(p,"legacy",bi,false,params).runTask(PlayerStatueBuilder.instance);
					} else if (flags.contains("default")) {
						new PlayerStatueMaker(p,"default",bi,false,params).runTask(PlayerStatueBuilder.instance);
					}
				} else {
					new PlayerStatueMaker(p,"default",bi,false,params).runTask(PlayerStatueBuilder.instance);
				}
